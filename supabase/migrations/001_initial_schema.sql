...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  -- 管理员控制：该用户是否需要付费（false = 免费使用）
  paywall_enabled      BOOLEAN NOT NULL DEFAULT TRUE,
  -- 管理员为该用户自定义试用天数（NULL = 使用全局默认）
...
CREATE TABLE IF NOT EXISTS public.subscriptions (
  ...
  expires_at   TIMESTAMPTZ,              -- NULL = 永不过期（lifetime/unlimited）
  granted_by   UUID REFERENCES public.profiles(id),  -- 管理员授予时非 NULL
  notes        TEXT,
  created_at   TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.app_versions (
  ...
  release_notes   TEXT,
  is_force        BOOLEAN GENERATED ALWAYS AS (
    -- 通过比较 min_version 是否大于 latest_version 的前一版本来决定是否强制
    FALSE
  ) STORED,
  created_at      TIMESTAMPTZ DEFAULT NOW()
);
