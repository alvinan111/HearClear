  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">版本管理</h2>
          <p className="text-zinc-500 text-sm mt-1">管理 App 更新推送</p>
        </div>
        <button onClick={() => setShowForm(true)} className="btn-primary">+ 发布新版本</button>
      </div>

      {showForm && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#0f0f11] p-6 mb-6">
          <h3 className="font-semibold text-white mb-5">发布新版本</h3>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">平台</label>
              <select value={form.platform} onChange={e => setForm({ ...form, platform: e.target.value })} className="input-base">
                <option value="android">🤖 Android</option>
                <option value="ios">🍎 iOS</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1.5">最新版本号</label>
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {versions.map((v) => (
                <tr key={v.id} className="hover:bg-white/[0.03] transition-colors">
                  <td className="table-td"><span className={`badge ${v.platform === 'ios' ? 'bg-zinc-700 text-white' : 'badge-green'}`}>{v.platform === 'ios' ? '🍎 iOS' : '🤖 Android'}</span></td>
                  <td className="table-td font-mono font-bold text-cyan-400">{v.latest_version}</td>
                  <td className="table-td"><span className="badge badge-red font-mono">{v.min_version}</span></td>
                  <td className="table-td text-zinc-500 max-w-xs truncate">{v.release_notes ?? '—'}</td>
                  <td className="table-td text-zinc-500 text-xs">{new Date(v.created_at).toLocaleString('zh-CN')}</td>
                </tr>
              ))}
              {versions.length === 0 && <tr><td colSpan={5} className="text-center text-zinc-500 py-16 text-sm">暂无版本记录</td></tr>}
            </tbody>
          </table>
        )}
      </div>
