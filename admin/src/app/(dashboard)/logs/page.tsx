  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">所有管理员操作记录</p>
      </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {logs.map((l) => (
                <tr key={l.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="table-td font-mono text-xs text-indigo-600">{l.profiles?.phone ?? l.admin_id.slice(0, 8)}</td>
                  <td className="table-td">
                    <span className={`badge ${ACTION_COLOR[l.action] ?? 'badge-gray'}`}>{l.action}</span>
                  </td>
                  <td className="table-td text-slate-400 text-xs">
                    {l.target_type && <span className="text-slate-500 font-medium mr-1">{l.target_type}:</span>}
                    {l.target_id?.slice(0, 12)}
                  </td>
                  <td className="table-td text-slate-400 text-xs max-w-xs truncate">
                    {l.details ? JSON.stringify(l.details) : '—'}
                  </td>
                  <td className="table-td text-slate-400 text-xs">{new Date(l.created_at).toLocaleString('zh-CN')}</td>
                </tr>
              ))}
              {logs.length === 0 && <tr><td colSpan={5} className="text-center text-slate-400 py-16 text-sm">暂无日志</td></tr>}
            </tbody>
          </table>
        )}
      </div>
