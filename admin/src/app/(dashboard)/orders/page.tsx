    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">共 {orders.length} 笔订单 · 累计收入 ¥{(totalRevenue / 100).toFixed(2)}</p>
        </div>
      </div>
              ...
              {orders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="hover:bg-white/[0.03] transition-colors">
                    <td className="table-td font-mono text-xs text-zinc-300">{o.profiles?.phone ?? o.user_id.slice(0, 8)}</td>
                    <td className="table-td"><span className="badge badge-blue">{o.subscription_type}</span></td>
                    <td className="table-td font-semibold text-white">¥{(o.amount_cents / 100).toFixed(2)}</td>
                    <td className="table-td text-zinc-400">{o.channel === 'wechat' ? '💚 微信' : '💙 支付宝'}</td>
                    <td className="table-td"><span className={`badge ${cls}`}>{label}</span></td>
                    <td className="table-td text-zinc-500 text-xs">{o.paid_at ? new Date(o.paid_at).toLocaleString('zh-CN') : '—'}</td>
                    <td className="table-td text-zinc-500 text-xs">{new Date(o.created_at).toLocaleString('zh-CN')}</td>
                  </tr>
                );
              })}
              {orders.length === 0 && <tr><td colSpan={7} className="text-center text-zinc-500 py-16 text-sm">暂无订单数据</td></tr>}
            </tbody>
          </table>
        )}
      </div>
